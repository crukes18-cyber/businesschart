          ...
          <div style={{ background: "#fff", borderRadius: "0 0 12px 12px", border: "1px solid #e8e8e4", overflow: "hidden", minWidth: 900 }}>
            {filtered.map((r, idx) => {
              ...
              <div key={r.id} style={{ display: "flex", alignItems: "stretch", borderBottom: "1px solid #f0f0ec", cursor: "pointer",
                ...
                <div style={S.cell(90)} onClick={() => openDetail(r)}><span style={{ fontWeight: 600, color: "#2C3E50", fontSize: 12 }}>{r.brand}</span></div>
                ...
                <div style={S.cellF} onClick={() => openDetail(r)}>
                  <span style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontSize:12 }}>{r.content}</span>
                </div>
                ...
                <div style={{ ...S.cellF, maxWidth: 120 }} onClick={() => openDetail(r)}><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11, color: "#666" }}>{r.nextStep || "—"}</span></div>
                <div style={S.cell(90)} onClick={() => openDetail(r)}><span style={{ fontSize:10, color:"#aaa" }}>{r.updatedAt || "—"}</span></div>
                <div style={S.cell(60)} onClick={() => openDetail(r)}><span style={{ fontSize:11, color:"#2C3E50", fontWeight:600 }}>{r.updatedBy || "—"}</span></div>
                <div style={S.cell(70)}>
